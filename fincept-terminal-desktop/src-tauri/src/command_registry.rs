// Shared command registry for Tauri + Axum builds.
// The list here is the single source of truth for command handlers.
...
#[macro_export]
macro_rules! tauri_command_handler_list {
    ($($handler:path),* $(,)?) => {
        $($handler),*
    };
}
...
#[macro_export]
macro_rules! tauri_command_path_list {
    ($($handler:path),* $(,)?) => {
        &[$(stringify!($handler)),*]
    };
}

pub const TAURI_COMMAND_PATHS: &[&str] = crate::for_each_tauri_command!(tauri_command_path_list);
